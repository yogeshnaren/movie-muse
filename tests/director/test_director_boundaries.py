"""Hosts import movie_muse.director.api; internals are rejected."""
...
def test_host_importing_director_api_is_allowed(tmp_path: Path) -> None:
    source = tmp_path / "backend" / "app" / "director_routes.py"
...
@pytest.mark.architecture
def test_host_importing_director_internal_is_rejected(tmp_path: Path) -> None:
    source = tmp_path / "backend" / "app" / "director_routes.py"
    source.parent.mkdir(parents=True)
    source.write_text(
...
@pytest.mark.architecture
def test_director_package_imports_only_public_sibling_apis() -> None:
    package = repo_root() / "src" / "movie_muse" / "director"
    siblings = (
        "audit",
        "authorization",
        "creative_intent",
    ...
    for path in package.glob("*.py"):
        ...
        assert "from movie_muse.shot_ir" not in text
