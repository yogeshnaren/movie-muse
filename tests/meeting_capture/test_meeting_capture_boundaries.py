"""Hosts import movie_muse.meeting_capture.api; internals are rejected."""
...
def test_host_importing_meeting_capture_api_is_allowed(tmp_path: Path) -> None:
    source = tmp_path / "backend" / "app" / "meeting_routes.py"
...
@pytest.mark.architecture
def test_host_importing_meeting_capture_internal_is_rejected(tmp_path: Path) -> None:
    source = tmp_path / "backend" / "app" / "meeting_routes.py"
    source.parent.mkdir(parents=True)
    source.write_text(
...
@pytest.mark.architecture
def test_meeting_capture_package_imports_only_public_sibling_apis() -> None:
    package = repo_root() / "src" / "movie_muse" / "meeting_capture"
    siblings = (
        "artifacts",
        "audit",
        "authorization",
