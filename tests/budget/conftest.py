"""Builders for budget tests. Duplicated rather than imported."""
...
class MutableClock:
    def __init__(self) -> None:
        self.value = datetime(2026, 9, 19, 1, 20, tzinfo=UTC)
...
def boot_budget_stack(root: Path) -> BudgetStack:
...
def compile_schedule(stack: BudgetStack):
...
@pytest.fixture
def budget_stack(tmp_path: Path) -> BudgetStack:
    return boot_budget_stack(tmp_path / "ws")


@pytest.fixture
def compiled_schedule(budget_stack: BudgetStack):
    return compile_schedule(budget_stack)
