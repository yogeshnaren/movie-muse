"""Hosts import movie_muse.shot_ir.api; internals are rejected."""
...
def test_host_importing_shot_ir_api_is_allowed(tmp_path: Path) -> None:
    source = tmp_path / "backend" / "app" / "shot_routes.py"
...
@pytest.mark.architecture
def test_host_importing_shot_ir_internal_is_rejected(tmp_path: Path) -> None:
    source = tmp_path / "backend" / "app" / "shot_routes.py"
    source.parent.mkdir(parents=True)
    source.write_text(
        "from movie_muse.shot_ir.service import ShotIRService\n",
        encoding="utf-8",
    )
...
@pytest.mark.architecture
def test_shot_ir_package_imports_only_public_sibling_apis() -> None:
    package = repo_root() / "src" / "movie_muse" / "shot_ir"
    siblings = (
        "audit",
        "authorization",
        "dependencies",
