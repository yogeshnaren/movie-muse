"""Hosts import movie_muse.scheduling.api; internals are rejected."""
...
def test_host_importing_scheduling_api_is_allowed(tmp_path: Path) -> None:
    source = tmp_path / "backend" / "app" / "schedule_routes.py"
...
@pytest.mark.architecture
def test_host_importing_scheduling_internal_is_rejected(tmp_path: Path) -> None:
    source = tmp_path / "backend" / "app" / "schedule_routes.py"
    source.parent.mkdir(parents=True)
    source.write_text(
...
@pytest.mark.architecture
def test_scheduling_package_imports_only_public_sibling_apis() -> None:
    package = repo_root() / "src" / "movie_muse" / "scheduling"
    siblings = (
        "audit",
        "authorization",
        "breakdown",
