"""Hosts import movie_muse.beats.api; internals are rejected."""
...
def test_host_importing_beats_api_is_allowed(tmp_path: Path) -> None:
    source = tmp_path / "backend" / "app" / "beat_routes.py"
...
@pytest.mark.architecture
def test_host_importing_beats_internal_is_rejected(tmp_path: Path) -> None:
    source = tmp_path / "backend" / "app" / "beat_routes.py"
    source.parent.mkdir(parents=True)
    source.write_text(
        "from movie_muse.beats.service import BeatService\n",
        encoding="utf-8",
    )
...
@pytest.mark.architecture
def test_beats_package_imports_only_public_sibling_apis() -> None:
