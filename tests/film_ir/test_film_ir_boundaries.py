"""Hosts import movie_muse.film_ir.api; internals are rejected."""
...
def test_host_importing_film_ir_api_is_allowed(tmp_path: Path) -> None:
    source = tmp_path / "backend" / "app" / "film_ir_routes.py"
...
@pytest.mark.architecture
def test_host_importing_film_ir_internal_is_rejected(tmp_path: Path) -> None:
    source = tmp_path / "backend" / "app" / "film_ir_routes.py"
    source.parent.mkdir(parents=True)
    source.write_text(
...
@pytest.mark.architecture
def test_film_ir_package_imports_only_public_sibling_apis() -> None:
    package = repo_root() / "src" / "movie_muse" / "film_ir"
    siblings = (
        "authorization",
        "compiler",
        "context",
