"""Hosts import movie_muse.context.api; internals are rejected."""
...
def test_host_importing_context_api_is_allowed(tmp_path: Path) -> None:
    source = tmp_path / "backend" / "app" / "context_routes.py"
...
@pytest.mark.architecture
def test_host_importing_context_internal_is_rejected(tmp_path: Path) -> None:
    source = tmp_path / "backend" / "app" / "context_routes.py"
    source.parent.mkdir(parents=True)
    source.write_text(
...
@pytest.mark.architecture
def test_context_package_imports_only_public_sibling_apis() -> None:
    package = repo_root() / "src" / "movie_muse" / "context"
    siblings = (
        "audit",
        "authorization",
        "identity",
