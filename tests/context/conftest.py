"""Builders for context tests. Duplicated rather than imported from other packages."""
...
from movie_muse.context.api import ContextService
...
def boot_context_stack(
    ...
    context = ContextService(revisions, retrieval, authorization)
    return ContextStack(
        workspace=workspace,
        identity=identity,
...
def invite_role(stack: ContextStack, role: Role):
...
def register_licensed_source(stack: ContextStack, *, title: str = "Licensed kitchen notes"):
...
@pytest.fixture
def context_stack(tmp_path: Path) -> ContextStack:
    return boot_context_stack(tmp_path / "ws", make_project_and_document())


@pytest.fixture
def licensed_source(context_stack: ContextStack):
    return register_licensed_source(context_stack)
