"""Builders for storyboard tests. Duplicated rather than imported."""
...
class MutableClock:
    def __init__(self) -> None:
        self.value = datetime(2026, 9, 19, 3, 0, tzinfo=UTC)
...
def boot_storyboard_stack(root: Path) -> StoryboardStack:
...
def open_space(stack: StoryboardStack, scene_id: str | None = None):
...
def create_shot(stack: StoryboardStack):
...
def invite_role(stack: StoryboardStack, role: Role, *, integration: bool = False):
...
@pytest.fixture
def storyboard_stack(tmp_path: Path) -> StoryboardStack:
    return boot_storyboard_stack(tmp_path / "ws")


@pytest.fixture
def stored_shot(storyboard_stack: StoryboardStack):
    return create_shot(storyboard_stack)


@pytest.fixture
def member(storyboard_stack: StoryboardStack):
    def _member(role: Role, *, integration: bool = False):
        return invite_role(storyboard_stack, role, integration=integration)
