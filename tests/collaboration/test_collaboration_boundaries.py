"""Hosts import movie_muse.collaboration.api; internals are rejected."""
...
def test_host_importing_collaboration_api_is_allowed(tmp_path: Path) -> None:
    source = tmp_path / "backend" / "app" / "collab_routes.py"
...
@pytest.mark.architecture
def test_host_importing_collaboration_internal_is_rejected(tmp_path: Path) -> None:
    source = tmp_path / "backend" / "app" / "collab_routes.py"
    source.parent.mkdir(parents=True)
    source.write_text(
        "from movie_muse.collaboration.service import CollaborationService\n",
        encoding="utf-8",
    )
...
@pytest.mark.architecture
def test_collaboration_package_imports_only_public_sibling_apis() -> None:
    package = repo_root() / "src" / "movie_muse" / "collaboration"
    siblings = (
        "audit",
        "authorization",
        "document",
