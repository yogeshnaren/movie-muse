"""Hosts import movie_muse.commercial_forecast.api; internals are rejected."""
...
def test_host_importing_forecast_api_is_allowed(tmp_path: Path) -> None:
    source = tmp_path / "backend" / "app" / "forecast_routes.py"
...
@pytest.mark.architecture
def test_host_importing_forecast_internal_is_rejected(tmp_path: Path) -> None:
    source = tmp_path / "backend" / "app" / "forecast_routes.py"
    source.parent.mkdir(parents=True)
    source.write_text(
        "from movie_muse.commercial_forecast.service import CommercialForecastService\n",
        encoding="utf-8",
    )
...
@pytest.mark.architecture
def test_forecast_package_imports_only_public_sibling_apis() -> None:
    package = repo_root() / "src" / "movie_muse" / "commercial_forecast"
    siblings = (
        "audience_lab",
        "audit",
        "authorization",
    ...
    for path in package.glob("*.py"):
        ...
        assert "from tests.audience_lab" not in text
