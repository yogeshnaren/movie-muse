"""Hosts import movie_muse.api.api; internals are rejected."""
...
def test_host_importing_mesh_api_is_allowed(tmp_path: Path) -> None:
    source = tmp_path / "backend" / "app" / "mesh_routes.py"
...
@pytest.mark.architecture
def test_host_importing_mesh_internal_is_rejected(tmp_path: Path) -> None:
    source = tmp_path / "backend" / "app" / "mesh_routes.py"
    source.parent.mkdir(parents=True)
    source.write_text(
...
@pytest.mark.architecture
def test_api_package_imports_only_public_sibling_apis() -> None:
    package = repo_root() / "src" / "movie_muse" / "api"
    siblings = (
        "artifacts",
        "audit",
        "authorization",
