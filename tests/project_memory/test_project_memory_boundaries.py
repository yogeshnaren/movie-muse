"""Hosts import movie_muse.project_memory.api; internals are rejected."""
...
def test_host_importing_project_memory_api_is_allowed(tmp_path: Path) -> None:
    source = tmp_path / "backend" / "app" / "memory_routes.py"
...
@pytest.mark.architecture
def test_host_importing_project_memory_internal_is_rejected(tmp_path: Path) -> None:
    source = tmp_path / "backend" / "app" / "memory_routes.py"
    source.parent.mkdir(parents=True)
    source.write_text(
...
@pytest.mark.architecture
def test_project_memory_package_imports_only_public_sibling_apis() -> None:
    package = repo_root() / "src" / "movie_muse" / "project_memory"
    siblings = (
        "artifacts",
        "audit",
        "authorization",
