"""Content-addressed retrieval index stored through ``workspace_meta``."""
...
INDEX_META_KEY = "retrieval.index_digest"
...
def load_index(workspace: LocalWorkspace) -> dict[str, Any]:
    ...
    if not isinstance(decoded, dict) or decoded.get("schema_version") != INDEX_SCHEMA_VERSION:
        raise ValueError("unsupported retrieval index blob")
...
def mutate_index(
    ...
    """Serialize retrieval index changes under BEGIN IMMEDIATE. No audit here."""
...
def load_payload(workspace: LocalWorkspace, digest: str) -> dict[str, Any]:
    ...
    if not isinstance(decoded, dict):
        raise ValueError("retrieval entry blob is not an object")
    return decoded
