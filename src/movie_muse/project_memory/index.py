"""Content-addressed project-memory index stored through ``workspace_meta``."""
...
INDEX_META_KEY = "project_memory.index_digest"
...
def empty_index() -> dict[str, Any]:
    return {
        ...
        "candidate_ids_by_project": {},
        "candidate_digests": {},
        "memory_ids_by_project": {},
        "memory_digests": {},
    }
...
def load_index(workspace: LocalWorkspace) -> dict[str, Any]:
    ...
    if not isinstance(decoded, dict) or decoded.get("schema_version") != INDEX_SCHEMA_VERSION:
        raise ValueError("unsupported project_memory index blob")
...
def load_payload(workspace: LocalWorkspace, digest: str) -> dict[str, Any]:
    ...
    if not isinstance(decoded, dict):
        raise ValueError("project_memory blob is not an object")
    return decoded
