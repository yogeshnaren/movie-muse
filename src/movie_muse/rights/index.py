"""Content-addressed rights index stored through ``workspace_meta``."""
...
INDEX_META_KEY = "rights.index_digest"
...
def load_index(workspace: LocalWorkspace) -> dict[str, Any]:
    ...
    if not isinstance(decoded, dict) or decoded.get("schema_version") != INDEX_SCHEMA_VERSION:
        raise ValueError("unsupported rights index blob")
...
def mutate_index(
    ...
    """Serialize rights index changes and atomically replace its digest pointer."""
...
def load_payload(workspace: LocalWorkspace, digest: str) -> dict[str, Any]:
    ...
    if not isinstance(decoded, dict):
        raise ValueError("rights record blob is not an object")
    return decoded
