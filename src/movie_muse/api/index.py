"""Content-addressed Integration Mesh index stored through ``workspace_meta``."""
...
INDEX_META_KEY = "integration_mesh.index_digest"
...
def load_index(workspace: LocalWorkspace) -> dict[str, Any]:
    ...
    if not isinstance(decoded, dict) or decoded.get("schema_version") != INDEX_SCHEMA_VERSION:
        raise ValueError("unsupported integration mesh index blob")
...
def load_payload(workspace: LocalWorkspace, digest: str) -> dict[str, Any]:
    ...
    if not isinstance(decoded, dict):
        raise ValueError("integration mesh blob is not an object")
    return decoded
