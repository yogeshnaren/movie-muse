"""Content-addressed investor-artifact index stored through ``workspace_meta``."""
...
INDEX_META_KEY = "investor_artifacts.index_digest"
...
def load_index(workspace: LocalWorkspace) -> dict[str, Any]:
    ...
    if not isinstance(decoded, dict) or decoded.get("schema_version") != INDEX_SCHEMA_VERSION:
        raise ValueError("unsupported investor artifact index blob")
...
def load_payload(workspace: LocalWorkspace, digest: str) -> dict[str, Any]:
    ...
    if not isinstance(decoded, dict):
        raise ValueError("investor artifact blob is not an object")
    return decoded
