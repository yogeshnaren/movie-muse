"""Content-addressed Reference Lens settings stored through ``workspace_meta``."""
...
INDEX_META_KEY = "reference_lens.index_digest"
...
def load_index(workspace: LocalWorkspace) -> dict[str, Any]:
    ...
    if not isinstance(decoded, dict) or decoded.get("schema_version") != INDEX_SCHEMA_VERSION:
        raise ValueError("unsupported reference_lens index blob")
