"""Content-addressed commercial-forecast index stored through ``workspace_meta``."""
...
INDEX_META_KEY = "commercial_forecast.index_digest"
...
def load_index(workspace: LocalWorkspace) -> dict[str, Any]:
    ...
    if not isinstance(decoded, dict) or decoded.get("schema_version") != INDEX_SCHEMA_VERSION:
        raise ValueError("unsupported commercial forecast index blob")
...
def load_payload(workspace: LocalWorkspace, digest: str) -> dict[str, Any]:
    ...
    if not isinstance(decoded, dict):
        raise ValueError("commercial forecast blob is not an object")
    return decoded
